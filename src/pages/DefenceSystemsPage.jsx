...
const isSimulatorCategory = (category) =>
  category?.id === SIMULATOR_CATEGORY_ID ||
...
const DefenceSystemsPage = () => {
  ...
  useEffect(() => {
    const loadDefenceData = async () => {
      try {
        ...
        if (!categoryRes.ok || !productRes.ok) throw new Error("Failed to load data");

        const categoryData = await categoryRes.json();
        const productData = await productRes.json();

        const filteredCategories = (Array.isArray(categoryData) ? categoryData : []).filter(
          (category) => !isSimulatorCategory(category)
        );

        const allowedCategoryIds = new Set(filteredCategories.map((c) => c.id));

        const filteredProducts = (Array.isArray(productData) ? productData : []).filter(
          (product) =>
            allowedCategoryIds.has(product.category_id) &&
            product.category_id !== SIMULATOR_CATEGORY_ID &&
            !String(product.category_name || product.category || "").toLowerCase().includes("simulator")
        );
  ...
  const navCategories = useMemo(
    () => categories.map((category) => ({ ...category, sectionId: slugify(category.name) })),
    [categories]
  );
  ...
  return (
    <div className="min-h-screen bg-[#f5f4ef] text-[#18253a]">
      <style>{`
        ...
        .defence-soft-float { animation: softFloat 5s ease-in-out infinite; }
      `}</style>
      ...
      <main>
        ...
        {loading && (
          <section className="py-20 bg-[#f5f4ef]">
            <div className="flex items-center justify-center gap-3 text-[#2e4a39]">
              ...
              <span className="text-lg font-semibold">Loading defence systems...</span>
            </div>
          </section>
        )}
        ...
        {!loading &&
          ...
          categories.map((category, categoryIndex) => {
            ...
            return (
              <section key={category.id} id={sectionId} className="scroll-mt-28 py-16 bg-[#f5f4ef]">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  <Reveal delay={categoryIndex * 80}>
                    <div className="border-b border-[#d8d6cf] pb-8 mb-12">
                      <div className="inline-flex items-center gap-2 px-3 py-2 bg-[#e9ece8] border border-[#d3d8d1] mb-4">
                        ...
                        <span className="text-sm font-semibold text-[#2e4a39]">{category.name}</span>
                      </div>
                  ...
                  {categoryProducts.length === 0 ? (
                    <Reveal>
                      <p className="text-[#677383]">No products available in this category.</p>
                    </Reveal>
                  ) : (
